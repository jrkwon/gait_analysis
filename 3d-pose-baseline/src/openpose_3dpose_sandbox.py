...
def read_openpose_json(smooth=True, *args):
    ...
    if len(json_files) == 0:
        logger.info(openpose_output_dir)
        logger.info('check the openpose dir')
        sys.exit(-1)
    cache = {}
    smoothed = {}
    ...
    for file_name in json_files:
        ...
        # occlusion case
        #0723 zero value
        if _data[9] == 0 and _data[10] == 0:
            _data[9] = _data[6]
            _data[10] = _data[7]
        if _data[12] == 0 and _data[13] == 0:
            _data[12] = _data[9]
            _data[13] = _data[10]
...
joint_index = [0,1,2,3,6,7,8,12,13,14,15,17,18,19,25,26,27]
joint_name = ['Hip','RHip','RKnee','RFoot','LHip','LKnee','LFoot','Spine','Thorax','Neck/Nose','Head','LShoulder','LElbow','LWrist','RShoulder','RElbow','RWrist']
def main(_):
    before_pose = 0
    ...
    if not '3d-baseline' in os.listdir(FLAGS.openpose+'../'):
        os.mkdir(FLAGS.openpose+'../3d-baseline/')
    ...
    with tf.Session(config=tf.ConfigProto(
        ...
        for n, (frame, xy) in enumerate(smoothed.items()):
            ...
            # save 3d joint
            json_name=FLAGS.openpose+'../3d-baseline/frame_{0}.json'.format(str(frame).zfill(12))
            json_val = {}
            for e_i, idx in enumerate(joint_index):
                json_val[joint_name[e_i]] = [poses3d[0][idx*3+0],poses3d[0][idx*3+1],poses3d[0][idx*3+2]]
            ...
            if np.min(poses3d) < -1000 and before_pose != -1:
                poses3d = before_pose
